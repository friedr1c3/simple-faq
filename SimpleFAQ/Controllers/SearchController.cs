...
namespace SimpleFAQ.Controllers
{
    ...
    public class SearchController : BaseController
	{
		...
		public ActionResult Index(string input, int? page, int? pageSize)
        {
			...
			pageSize = Math.Max(Math.Min(pageSize ?? 50, 100), 10); // Adjust.
			...
			counter = builder.AddTemplate("SELECT COUNT(*) FROM Questions /**where**/");
			...
			var totals = Current.DB.Query<int>(counter.RawSql, counter.Parameters);

			return View(new PagedList<Question>(results, page.Value, pageSize.Value, false, 0));
		}
